...
const Navbar = () => {
  ...
  return (
    ...
    >
                ...
                <li
                  ...
                  className={`${
                    active === link.title
                      ? "text-white font-bold"
                      : "text-secondary"
                  } font-poppins font-medium cursor-pinter text-[16px]`}
                  onClick={() => {
                    setToggle(!toggle);
                    setActive(link.title);
                  }}
      ...
      {/* Responsive Menu */}
      {toggle && (
        <div className="sm:hidden absolute top-20 right-0 left-0 bg-primary z-10">
          <ul className="text-white text-center py-4">
            {navLinks.map((link) => (
              <li
                key={link.id}
                className="hover:text-secondary text-[18px] py-2 cursor-pointer"
                onClick={() => {
                  setActive(link.title);
                  setToggle(false);
                }}
